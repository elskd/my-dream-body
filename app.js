(function () {
  ...
  function shell(content) {
    ...
    return '<div class="appShell">' +
      ...
      '<main class="pageMain"><header class="topHeader"><button class="searchBox" onclick="MDB.search()"><span>⌕</span><span>Найти в My Dream Body</span></button><div class="headerActions">' + iconButton("♡", "Избранное", "MDB.toggleFavorite()") + iconButton("♧", "Уведомления", "MDB.toast('Уведомления пока пусты')") + '<button class="profileMini" onclick="MDB.go(\'settings\')"><span>' + esc(state.profile.name.charAt(0)) + '</span>' + esc(state.profile.name) + '</button></div></header>' +
  ...
  function quickAdd(kind){if(kind==="dish"){modal("Новое блюдо",[{label:"Название",name:"name",required:true},{label:"Калории",name:"kcal",type:"number",required:true},{label:"Белки",name:"p",type:"number"},{label:"Жиры",name:"f",type:"number"},{label:"Углеводы",name:"c",type:"number"},{label:"Категория",name:"tag"}],function(f){state.dishes.push({id:"d"+Date.now(),name:f.get("name"),kcal:+f.get("kcal")||0,p:+f.get("p")||0,f:+f.get("f")||0,c:+f.get("c")||0,tag:f.get("tag")||"Моё блюдо",image:images.bowl});save();render();});return;}if(kind==="product"){modal("Новый продукт",[{label:"Название",name:"name",required:true},{label:"Калории",name:"kcal",type:"number",required:true},{label:"Белки",name:"p",type:"number"},{label:"Жиры",name:"f",type:"number"},{label:"Углеводы",name:"c",type:"number"},{label:"Категория",name:"tag"}],function(f){var id="p"+Date.now();state.products.push({id:id,name:f.get("name"),kcal:+f.get("kcal")||0,p:+f.get("p")||0,f:+f.get("f")||0,c:+f.get("c")||0,tag:f.get("tag")||"Продукт",image:images.bowl});state.fridge.push({id:id,have:true});save();render();});return;}if(kind==="exercise"){modal("Новое упражнение",[{label:"Название",name:"name",required:true},{label:"Группа мышц",name:"muscle"},{label:"Оборудование",name:"equipment"},{label:"Подходы",name:"sets",type:"number"},{label:"Повторения",name:"reps",type:"number"}],function(f){state.exercises.push({id:"e"+Date.now(),name:f.get("name"),muscle:f.get("muscle"),equipment:f.get("equipment"),sets:+f.get("sets")||3,reps:+f.get("reps")||10});save();});return;}
  ...
  function toast(msg){var old=document.querySelector(".mdbToast");if(old)old.remove();document.body.insertAdjacentHTML("beforeend",'<div class="mdbToast">'+esc(msg)+'</div>');setTimeout(function(){var x=document.querySelector(".mdbToast");if(x)x.remove();},2200);}
  function toggleFavorite(){toast("Добавлено в избранное");}
  function search(){modal("Поиск",[{label:"Что найти?",name:"q",required:true}],function(f){var q=String(f.get("q")||"").toLowerCase();var hits=state.dishes.concat(state.products,state.exercises,state.workouts).filter(function(x){return String(x.name||"").toLowerCase().indexOf(q)>-1;});toast(hits.length?"Найдено: "+hits.length:"Ничего не найдено");});}
  function setProgressRange(btn,range){document.querySelectorAll(".progressTabs button").forEach(function(x){x.classList.remove("active")});btn.classList.add("active");toast(range==="week"?"Показана неделя":range==="month"?"Показан месяц":"Показан год");}
  ...
  window.MDB={go:go,setSub:setSub,eat:eat,doneWorkout:doneWorkout,toggleFridge:toggleFridge,addDish:addDish,addMealPrompt:addMealPrompt,quickAdd:quickAdd,planWorkout:planWorkout,saveProfile:saveProfile,saveTargets:saveTargets,toggleSection:toggleSection,closeModal:closeModal,toast:toast,toggleFavorite:toggleFavorite,search:search,setProgressRange:setProgressRange};
  render();
})();