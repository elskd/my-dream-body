(function () {
  ...
  function shell(content) {
    ...
    return '<div class="appShell">' +
      ...
      '<main class="pageMain"><header class="topHeader"><button class="searchBox" onclick="MDB.search()">⌕ <span>Найти в My Dream Body</span></button><div class="headerActions">' + iconButton("♡", "Избранное") + iconButton("♧", "Уведомления") + '<button class="profileMini" onclick="MDB.go(\'settings\')"><span>' + esc(state.profile.name.charAt(0)) + '</span>' + esc(state.profile.name) + '</button></div></header>' +
  ...
  function createDish(){modal("Новое блюдо",[{label:"Название",name:"name",required:true},{label:"Калории",name:"kcal",type:"number",required:true},{label:"Белки",name:"p",type:"number"},{label:"Жиры",name:"f",type:"number"},{label:"Углеводы",name:"c",type:"number"},{label:"Категория",name:"tag"}],function(f){state.dishes.push({id:"d"+Date.now(),name:f.get("name"),kcal:+f.get("kcal")||0,p:+f.get("p")||0,f:+f.get("f")||0,c:+f.get("c")||0,tag:f.get("tag")||"Моё блюдо",image:images.bowl});save();render();});}
  function createProduct(){modal("Новый продукт",[{label:"Название",name:"name",required:true},{label:"Калории",name:"kcal",type:"number",required:true},{label:"Белки",name:"p",type:"number"},{label:"Жиры",name:"f",type:"number"},{label:"Углеводы",name:"c",type:"number"},{label:"Категория",name:"tag"}],function(f){var id="p"+Date.now();state.products.push({id:id,name:f.get("name"),kcal:+f.get("kcal")||0,p:+f.get("p")||0,f:+f.get("f")||0,c:+f.get("c")||0,tag:f.get("tag")||"Продукт",image:images.bowl});state.fridge.push({id:id,have:true});save();render();});}
  function toast(msg){var x=document.querySelector(".mdbToast");if(x)x.remove();document.body.insertAdjacentHTML("beforeend",'<div class="mdbToast">'+esc(msg)+'</div>');setTimeout(function(){var y=document.querySelector(".mdbToast");if(y)y.remove();},2200);}
  function search(){modal("Поиск",[{label:"Что найти?",name:"q",required:true}],function(f){var q=String(f.get("q")||"").toLowerCase();var hits=state.dishes.concat(state.products,state.exercises,state.workouts).filter(function(x){return String(x.name||"").toLowerCase().indexOf(q)>=0;});toast(hits.length+" результатов");});}
  function quickAdd(kind){if(kind==="dish"){createDish();return;}if(kind==="product"){createProduct();return;}if(kind==="exercise"){modal("Новое упражнение",[{label:"Название",name:"name",required:true},{label:"Группа мышц",name:"muscle"},{label:"Оборудование",name:"equipment"},{label:"Подходы",name:"sets",type:"number"},{label:"Повторения",name:"reps",type:"number"}],function(f){state.exercises.push({id:"e"+Date.now(),name:f.get("name"),muscle:f.get("muscle"),equipment:f.get("equipment"),sets:+f.get("sets")||3,reps:+f.get("reps")||10});save();});return;}
  ...
  window.MDB={go:go,setSub:setSub,eat:eat,doneWorkout:doneWorkout,toggleFridge:toggleFridge,addDish:addDish,addMealPrompt:addMealPrompt,quickAdd:quickAdd,planWorkout:planWorkout,saveProfile:saveProfile,saveTargets:saveTargets,toggleSection:toggleSection,closeModal:closeModal,search:search,toast:toast,range:function(btn){document.querySelectorAll(".progressTabs button").forEach(function(x){x.classList.remove("active");});btn.classList.add("active");}};
  render();
})();